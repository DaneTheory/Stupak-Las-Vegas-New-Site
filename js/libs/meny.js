	...
	create: function( options ) {
		return (function(){
			...
			var config = {
				...
				gradient: 'rgba(0, 0, 0, 0.1) 0%, rgba(243, 243, 243, 0.4) 3%, rgba(0, 0, 0, 0.48) 25%, rgba(0, 0, 0, .7) 45%, rgba(0, 0, 0, 1) 100%)',
				mouse: true,
				touch: true
			};
			...
			function setupPositions() {
				...
				contentsAngle = config.angle / ( 10 * window.innerWidth );
				...
				switch( config.position ) {
					...
					default:
						// Primary transform:
						menuTransformOrigin = '100% 0';
						menuTransformClosed = 'translateX( -100% ) translateX( '+ config.overlap +'px ) scale( 1 ) rotateY( ' + ( -menuAngle / -2 ) + 'deg )';
						contentsTransformOrigin = '-25% 50%';
						contentsTransformOpened = 'translateX( '+ config.width +'px ) scale( 0.9 ) rotateY( ' + -contentsAngle + 'deg )';
			...
			function setupWrapper() {
				...
				dom.wrapper.style[ Meny.prefix( 'perspective' ) ] = '1200px';
			...
			function setupMenu() {
				...
				if( supports3DTransforms ) {
					...
					style[ Meny.prefix( 'transition' ) ] = 'all ' + '0.5s' +' '+ 'ease';
				}
				else {
			...
			function setupContents() {
				...
				if( supports3DTransforms ) {
					...
					style[ Meny.prefix( 'transition' ) ] = 'all ' + '0.32s' +' '+ 'ease-in-out';
				}
				else {
